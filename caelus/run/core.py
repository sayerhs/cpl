...
def is_caelus_casedir(root=None):
    ...
    return all(os.path.exists(os.path.join(cdir, d)) for d in casedir_entries)
...
def find_caelus_recipe_dirs(basedir, action_file="caelus_tasks.yaml"):
...
def clean_polymesh(casedir, region=None, preserve_patterns=None):
    ...
    meshdir = (
        os.path.join(absdir, "constant", "polyMesh")
        if region is None
        else (os.path.join(absdir, "constant", region, "polyMesh"))
    )
    ...
    if os.path.exists(meshdir):
        ...
        _lgr.warning("No polyMesh directory %s; skipping clean_mesh", meshdir)
...
def clean_casedir(
    ...
    time_pat = ["[1-9]*", "0.[0-9]*", "-[0-9]*"] if preserve_times else []
    proc_pat = ["processor*"] if preserve_processors else []
    extra_pat = preserve_extra if preserve_extra else []
    ppatterns = base_patterns + zero_pat + extra_pat + time_pat + proc_pat
...
def clone_case(
    ...
    _lgr.info("Cloned directory: %s; template directory: %s", absdir, tmpl_dir)
    return absdir
