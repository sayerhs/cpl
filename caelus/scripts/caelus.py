...
from ..run import cmd
from ..run.core import clean_casedir, clone_case, get_mpi_size
...
class CaelusCmd(CaelusSubCmdScript):
    ...
    def cli_options(self):
        ...
        clone.add_argument(
            "template_dir", help="Valid Caelus case directory to clone."
        )
        clone.add_argument("case_name", help="Name of the new case directory.")
        ...
        # Run action
        run.add_argument(
            '-p', '--parallel', action='store_true', help="run in parallel"
        )
        run.add_argument(
            '-l',
            '--log-file',
        ...
        run.add_argument('cmd_name', help="name of the Caelus executable")
        run.add_argument(
            'cmd_args', nargs='*', help="additional arguments passed to command"
        )
        ...
        runpy.add_argument('script', help="path to the python script")
        runpy.add_argument(
            'script_args',
            nargs='*',
        ...
        logs.add_argument("log_file", help="log file (e.g., simpleSolver.log)")
        ...
        build.add_argument(
            '-c', '--clean', action='store_true', help="clean CML build"
        )
        build.add_argument(
        ...
        build.add_argument(
            'build_args', nargs='*', help="additional arguments passed to SCons"
        )
    ...
    def write_config(self):
        ...
        try:
            ...
            fh.close()
        if args.config_file:
            _lgr.info("CPL configuration written to file: %s", args.config_file)
    ...
    def run_cmd(self):
        ...
        if args.parallel:
            ...
            if args.machinefile is not None:
                if not os.path.exists(args.machinefile):
                    _lgr.error("Cannot find machine file: %s", args.machinefile)
                    self.parser.exit(1)
            ...
            _lgr.info(
                "Executing %s in parallel on %d ranks",
                args.cmd_name,
                cml_cmd.num_mpi_ranks,
            )
        else:
        ...
        if status != 0:
            _lgr.error(
                "Error executing command; see %s for details",
                cml_cmd.output_file,
            )
            self.parser.exit(status)
        else:
            _lgr.info(
                "Command executed successfully; see %s for output",
                cml_cmd.output_file,
            )
    ...
    def run_python(self):
        ...
        if status != 0:
            _lgr.error("Error executing python script: %s%s", pyscript, msg_str)
            self.parser.exit(status)
        else:
    ...
    def clone_case(self):
        ...
        if not (os.path.exists(template_dir) and os.path.isdir(template_dir)):
            _lgr.fatal("Cannot find template directory: %s", template_dir)
            self.parser.exit(1)
        if not (os.path.exists(args.base_dir) and os.path.isdir(args.base_dir)):
    ...
    def process_logs(self):
        ...
        args = self.args
        if not (os.path.exists(args.case_dir) and os.path.isdir(args.case_dir)):
        ...
        if args.plot_residuals:
            ...
            fields_filtered = [fld for fld in clog.fields if fld in fields]
            ...
            plot.plot_residuals_hist(plotfile=fname, fields=fields_filtered)
            _lgr.info("Residual time history saved to %s", args.plot_file)
    ...
    def cml_build(self):
        ...
        if build_project:
            ...
            if builder.rcode != 0:
                _lgr.error(
                    "Compilation failed in project directory. See log for details."
                )
        ...
        if build_user and prj_successful:
            ...
            if not status or builder.rcode != 0:
                _lgr.error(
                    "Compilation failed in user directory. See log for details."
                )
            else:
                _lgr.info("User sources compiled successfully.")
        elif build_user:
            _lgr.warning(
                "Detected incompleted project build; skipping user source compilation."
            )
        ...
        if build_srcdir:
            ...
            if builder.rcode != 0:
                _lgr.error(
                    "Compilation failed in directory. See log for details."
                )
            else:
